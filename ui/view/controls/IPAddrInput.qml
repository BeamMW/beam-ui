...
ColumnLayout {
    ...
    property var ipValidator: RegularExpressionValidator {regularExpression: /^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])$/g}
    property var ipDomainValidator: RegularExpressionValidator {regularExpression: /(^((?=.{0,255}$)(?!.*(_|(^-)|(\.-)|(-\.)).*)((([\w-]{1,63}\.)+[A-Za-z_][\w-]{0,61}[\w])|([\w-]{0,62}[\w])))$)|(^((?!(.*\.){4})(([0-1]?[\d]?[\d]|2[0-4][\d]|25[0-5])(\.|(?=$))){4})$)/g}
