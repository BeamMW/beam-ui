...
*/
    ...
    ProcessClientVersion(parameters, AppModel2::getMyName(), BEAM_CLIENT_VERSION, [this](const auto& version, const auto& myVersion)
    {
...
bool SendSwapViewModel::isEnough() const
{
    ...
    if (isEthereumBased(_sendCurrency))
    {
        if (_sendCurrency == Currency::CurrEthereum)
        {
            ...
            return AppModel2::getInstance().getSwapEthClient()->getAvailable(swapCoin) > total;
        }

        return AppModel2::getInstance().getSwapEthClient()->getAvailable(swapCoin) > _sendAmountGrothes &&
    ...
    return AppModel2::getInstance().getSwapCoinClient(swapCoin)->getAvailable() > total;
}
